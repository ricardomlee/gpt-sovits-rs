...
pub(crate) struct ConvLayerNorm {
    conv: Conv1d,
    norm: LayerNorm,
}

pub(crate) struct ConvLayerNoNorm {
    conv: Conv1d,
}

pub(crate) enum CnnLayer {
...
impl CnnLayer {
    pub fn norm_weight(&self) -> Option<&Tensor> {
        match self {
            Self::WithNorm(l) => Some(l.norm.weight()),
            Self::NoNorm(_) => None,
        }
    }
}
...
impl Wav2Vec2Model {
    pub fn load(vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..7 {
            ...
            if i == 0 {
                cnn.push(CnnLayer::load_with_norm(
                    vb_l, in_ch, 512, kernels[i], strides[i],
                )?);
            } else {
                cnn.push(CnnLayer::load_no_norm(
                    vb_l, in_ch, 512, kernels[i], strides[i],
                )?);
            }
        }
    ...
    fn forward_impl(
        ...
        audio: &Tensor,
        _debug: bool,
        _out: Option<&mut Vec<Tensor>>,
    ) -> Result<Tensor> {
        // Cast F32 input audio to model dtype (BF16 or F32)
        let audio = if self.dtype != audio.dtype() {
    ...
    pub fn cnn_norm_weight(&self) -> Result<Tensor> {
        self.cnn[0]
            .norm_weight()
            .ok_or_else(|| candle_core::Error::Msg("no norm".into()))?
            .clone()
            .flatten_all()
    }
    ...
    pub fn load_from_file_with_dtype(
        path: &std::path::Path,
        device: &Device,
        dtype: DType,
    ) -> crate::Result<Self> {
