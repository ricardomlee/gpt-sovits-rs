...
`--mode` 可选 `plain`、`kv`、`cuda-graph`，默认用稳定的 `kv`。`cuda-graph` 已通过 F32、300-token 长文本回归，但仍需覆盖更多显卡和模型；开发测试时设置 `GPT_SOVITS_EXPERIMENTAL_CUDA_GRAPH=1` 才会实际启用。实验路径会校验第一次 graph launch，发现结果偏离时从已校验的 KV 状态继续。CLI 日志会输出 `profile mode=... target=... ref=... target_bert=... gpt=... sovits=... total=...`，便于看时间花在哪里。
