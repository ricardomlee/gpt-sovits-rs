...
fn run_bench(pipeline: &mut Pipeline, label: &str) -> Vec<(usize, Duration, f32)> {
    ...
    for text in TEXTS {
        ...
        println!("  [{label}] {chars:2}chars  median={:.0}ms  audio={:.2}s  RTF={:.3}",
            median.as_secs_f64() * 1000.0, audio_dur_s, rtf);
    }
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  warmup={WARMUP}  timed_runs={RUNS}  texts={}\n", TEXTS.len());
    ...
    // --- Summary ---
    println!("\n{:-<60}", "");
    println!("{:>10}  {:>10}  {:>10}  {:>10}  {:>8}", "chars", "F32(ms)", "F16(ms)", "speedup", "RTF(F16)");
    println!("{:-<60}", "");
    ...
    for (i, ((chars, t32, _), (_, t16, aud16))) in r32.iter().zip(r16.iter()).enumerate() {
        ...
        println!("{:>10}  {:>10.0}  {:>10.0}  {:>9.2}x  {:>8.3}",
            chars,
