...
fn main() -> anyhow::Result<()> {
    ...
    println!("WAV: sr={}, ch={}, bits={}", spec.sample_rate, spec.channels, spec.bits_per_sample);
    ...
    println!("Samples: {} max={:.4}", samples.len(), samples.iter().cloned().fold(0.0_f32, f32::max));
    ...
    let dims = stft.dims();
    println!("Rust STFT: {:?} max={:.4}", dims, stft_data.iter().cloned().fold(0.0_f32, f32::max));
    ...
    let shape: Vec<usize> = shape_str.split(',').map(|s| s.trim().parse::<usize>().unwrap()).collect();
    ...
    let max_diff = (0..n).map(|i| (stft_data[i] - py_data[i]).abs()).fold(0.0_f32, f32::max);
    let mean_diff = (0..n).map(|i| (stft_data[i] - py_data[i]).abs()).sum::<f32>() / n as f32;
    println!("Spec comparison: max_diff={:.6}  mean_diff={:.6}", max_diff, mean_diff);
