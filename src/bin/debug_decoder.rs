...
fn test_single_conv() -> anyhow::Result<()> {
    ...
    let (_py_shape, py_data) = load_py_tensor("/tmp/py_resblock0_c1_out.txt")?;

    let max_diff = out_data
        .iter()
        .zip(py_data.iter())
        .map(|(r, p)| (r - p).abs())
    ...
    println!(
        "resblock0.convs1.1 max_diff={:.6} rust_max={:.4} py_max={:.4}",
        max_diff, rust_max, py_max
    );
