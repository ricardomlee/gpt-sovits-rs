...
Models are not bundled with the binary or Docker images. Prepare the official GPT-SoVITS v2 inference stack with:
...
```bash
python3 convert_gpt_weights.py /path/to/custom-gpt.ckpt models/gpt-model.safetensors
python3 convert_sovits_weights.py /path/to/custom-sovits.pth models/sovits-model.safetensors
```
...
```bash
python3 convert_sv_embedding.py /path/to/logs/voice_v2pro/7-sv_cn/ref.wav.pt voices/demo/ref_sv.safetensors
```

Trusted legacy checkpoints that contain Python pickle objects such as `utils.HParams` can be converted with `--allow-unsafe-pickle`. Read [docs/MODELS.md](docs/MODELS.md) before using it.
