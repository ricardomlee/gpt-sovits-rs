...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Reference audio: {} samples ({:.2}s) at {} Hz",
        ref_audio_buf.samples.len(), ref_audio_buf.duration(), ref_audio_buf.sample_rate);
    ...
    let samples_24k = if ref_audio_buf.sample_rate != 24000 {
        ...
        println!("  Resampling from {} Hz to 24000 Hz ({} → {} samples)",
            ref_audio_buf.sample_rate, ref_audio_buf.samples.len(), new_len);
        ...
        for i in 0..new_len {
            ...
            let v1 = ref_audio_buf.samples.get(src_idx + 1).copied().unwrap_or(0.0);
    ...
    let stft_mag = extractor.extract_spectrogram_batched(&samples_24k, &candle_core::Device::Cpu)?;
    ...
    match model.synthesize(&semantic_tokens, &text_tokens, Some(&stft_mag.to_device(model.device(), 0.5)?)) {
        ...
        Err(e) => {
            ...
            println!("  Note: enc_q requires SSL speaker embedding (512-dim) for proper voice cloning");
        }
    }
...
fn print_audio_stats(samples: &[f32]) {
    ...
    println!("  Finite: {}, NaN: {}, Inf: {}", finite_count, nan_count, inf_count);
    println!("  Min: {:.6}, Max: {:.6}, RMS: {:.6}", min, max, rms);

    let is_silent = samples.iter()
