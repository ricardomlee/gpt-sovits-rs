...
impl ToneSandhi {
    ...
    fn last_n_chars(word: &str, n: usize) -> String {
        word.chars().rev().take(n).collect::<Vec<_>>().into_iter().rev().collect()
    }
    ...
    fn yi_sandhi(word: &str, tones: &mut Vec<u32>) {
        ...
        if chars.iter().filter(|&&c| c != '一').all(|c| c.is_ascii_digit()) {
            return;
        }
        // V一V: e.g. 看一看 → "一" → neutral
        if chars.len() == 3 && chars.get(1) == Some(&'一') && chars[0] == chars[2] && tones.len() >= 2 {
            tones[1] = 5;
            return;
        }
        // 第一: "一" → tone 1
    ...
    fn neural_sandhi(&self, word: &str, pos: &str, tones: &mut Vec<u32>, jieba: &Jieba) {
        ...
        let second_to_last = if chars.len() >= 2 { chars.get(chars.len() - 2).copied() } else { None };
        let ge_char_idx: Option<usize> = chars.iter().position(|&c| c == '个');

        if last_char.map(|c| modal_particles.contains(c)).unwrap_or(false) {
            tones[n - 1] = 5;
        } else if last_char.map(|c| "的地得".contains(c)).unwrap_or(false) {
            tones[n - 1] = 5;
        } else if chars.len() == 1 && "了着过".contains(chars[0]) && matches!(pos, "ul" | "uz" | "ug") {
            tones[0] = 5;
        } else if chars.len() > 1
    ...
    fn merge_yi(seg: Vec<(String, String)>) -> Vec<(String, String)> {
        ...
        while i < orig_len {
            ...
            if i >= 1 && word == "一" && i + 1 < orig_len {
                let last = new_seg.last().cloned().unwrap_or_else(|| orig[i - 1].clone());
                let next = &orig[i + 1];
    ...
    fn merge_continuous_three_tones_2(seg: Vec<(String, String)>) -> Vec<(String, String)> {
        ...
        for i in 0..seg.len() {
            ...
            let prev_last_tone = if i >= 1 {
                tones_list[i - 1].iter().filter(|&&t| t > 0).last().copied().unwrap_or(0)
            } else {
                0
            };
